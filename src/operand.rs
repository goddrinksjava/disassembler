...
use crate::decode::AddressByteIteratorExt;
...
impl Operand {
    pub fn immediate<T>(w: u8, bytes: &mut Peekable<T>) -> anyhow::Result<[Operand; 2]>
    where
        T: Iterator<Item=(Address, u8)>,
    {
        ...
        if w == 0
        {
            ...
            Ok([op_rm, Operand::Immediate16(((data_hi as u16) << 8) | data_lo as u16)])
        }
    }

    pub fn from_mod_rm<T>(w: u8, mod_rm: u8, bytes: &mut Peekable<T>) -> anyhow::Result<Operand>
    where
        T: Iterator<Item=(Address, u8)>,
    {
        match mod_rm & 0b1100_0000 {
            0b0000_0000 => {
                ...
                Ok(Operand::Memory(Memory { displacement, registers }))
            }
            0b0100_0000 => {
                let (_, byte1) = bytes.try_next()?;
            ...
            0b1100_0000 => {
                Ok(Operand::Register(Register::decode_reg(mod_rm & 0b0000_0111, w)))
            }
            _ => unreachable!()
        }
    }

    pub fn from_mod_reg_rm<T>(d: u8, w: u8, bytes: &mut Peekable<T>) -> anyhow::Result<[Operand; 2]>
    where
        T: Iterator<Item=(Address, u8)>,
    {
